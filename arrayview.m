...
%        message
%
%   1) ndims checks for the number of dimensions - length(size(data)
%
%   2) use if and elseif to check for appropriate dimensions of data array
%           Take mean -> m = mean(data,4) across time
%
%   3) send error(msg) if input data is neither 3d or 4d

% data = niftiread('dafni_test.nii'); 

if ndims(data) == 3
    disp('h1: data set is 3d')
    
elseif ndims(data) == 4
    disp('h2: data set is 4d, take mean across time')
    disp('mean')
    m = mean(data,4) 
    
else error('data is neither 3d nor 4d')
